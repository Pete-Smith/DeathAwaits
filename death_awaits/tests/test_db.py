...
def test_insertion_start_end(test_database, test_data):
    ...
    assert row["quantity"] == (entry["end"] - entry["start"]).total_seconds() / 60
...
def test_simple_overlap(test_database, test_data):
    ...
    assert row_b["start"] + timedelta(seconds=row_b["quantity"] * 60) == row_b["end"]
...
def test_simple_span_slices(test_database, test_data):
    id_ = test_database.create_entry(**test_data[0])
    entry = test_database.row(id_)
    slices, activities = test_database.span_slices(
        start=entry["start"],
        span=entry["quantity"] * 2.0,
        slice_size=entry["quantity"] * 2.0,
        level=0,
        unrecorded=True,
    )
    assert activities["unrecorded"][0] == pytest.approx(0.5)
    assert len(slices) == 1
    assert slices[0] == entry["start"] + timedelta(minutes=entry["quantity"])
...
def test_fill_unrecorded_first(test_database, test_data):
    ...
    assert test_database.slice_activities(span[0], span[1], 1, True)[
        "unrecorded"
    ] == pytest.approx(0.25)
