...
class ActivityTilesBase(PlotDialogBase):
    nesting_types = (
        "Largest Inside",
        "Smallest Inside",
        "Largest Inside, By Tile",
        "Smallest Inside, By Tile",
    )
    ...
    def proportion_to_radius(value, full_radius=0.5):
        return math.sqrt((math.pi * full_radius**2) * value) / math.sqrt(math.pi)
    ...
    def _plot(self, figure, database, activity, start, end):
        ...
        with open(os.path.expanduser("~/Desktop/colors.txt"), "w") as f_out:
            f_out.writelines(
                ["{0} : {1}\n".format(k, v) for k, v in self._color_assignments.items()]
            )
    ...
    def process_activity_cluster(self, chunk_activities, allowed_activities, activity):
        activities = [
            (k, v) for k, v in chunk_activities.items() if k in allowed_activities
        ]
        reg = re.compile(activity, re.IGNORECASE)
        other_proportion = sum(
            v
            for k, v in chunk_activities.items()
            if k not in allowed_activities
            and (self.inclusive_other.isChecked() or reg.search(k))
        )
        if self.nesting_type.currentIndex() in (0, 1):
            ...
            activities.append(("other", other_proportion))
            activities.sort(
                key=lambda i: i[1], reverse=self.nesting_type.currentIndex() == 3
            )
        else:
...
class ActivityTilesHourly(ActivityTilesBase):
    ...
    def format_day_hours(value, pos):
        ...
        fmt_string = core.QLocale().timeFormat(core.QLocale.FormatType.ShortFormat)
    ...
    def _plot_data(self, axes, database, activity, start, end):
        ...
        yaxis.set_major_locator(matplotlib.ticker.MaxNLocator(11, integer=True))
        ...
        for day_offset in range((end - start).days):
            for hour_offset in range(24):
                ...
                patches.extend(
                    self.render_cell(day_offset, hour_offset, activities, axes, palette)
                )
...
class ActivityTilesDaily(ActivityTilesBase):
    ...
    def weekday_formatter(value, pos):
        locale = core.QLocale()
        days = [locale.dayName(n, core.QLocale.FormatType.ShortFormat) for n in range(1, 8)]
        while value < 0:
            value += 7
        while value > 7:
            value -= 7
    ...
    def _plot_data(self, axes, database, activity, start, end):
        ...
        yaxis.set_major_locator(
            matplotlib.ticker.FixedLocator([n + 0.5 for n in range(7)], 6)
        )
        start, end = self.bracket(database, "", start, end)
        # start and end should fall on day divisions.
        start = datetime.datetime.fromordinal(start.date().toordinal())
        ...
        number_of_weeks = ((end_year * 52) + end_week) - (
            (start_year * 52) + start_week
        )
        xaxis.set_major_formatter(
            matplotlib.ticker.FuncFormatter(self.create_week_format_func(start_week))
        )
        ...
        while start + datetime.timedelta(days=day_offset) < end:
            ...
            week_offset = ((current_year * 52) + current_week) - (
                (start_year * 52) + start_week
            )
            patches.extend(
                self.render_cell(
