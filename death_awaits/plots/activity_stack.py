...
class ActivityStack(PlotDialogBase):
    ...
    def additional_widgets(self):
        ...
        self.day_boundary_lines = widgets.QCheckBox("Day boundary lines.", self)
    ...
    def _plot(self, figure, database, activity, start, end):
        ...
        axes.xaxis.set_ticks(
            [
                ...
                for n in range(int(round((end - start) / datetime.timedelta(days=1))))
            ],
            minor=True,
        )
    ...
    def _graph_data(
        ...
        for name, values in activities.items():
            if name in allowed_activities and name.strip().lower() != "other":
...
class ActivityStackDay(ActivityStack):
    ...
    def _plot(self, figure, database, activity, start, end):
        ...
        if weekly:
            ...
            axes.xaxis.set_ticks(
                [datetime.timedelta(hours=h).total_seconds() for h in range(0, 30, 6)]
            )
            axes.xaxis.set_ticks(
                [datetime.timedelta(hours=h).total_seconds() for h in range(25)],
                minor=True,
            )
            axes.xaxis.set_ticklabels(("Midnight", "6am", "Noon", "6pm", "Midnight"))
        ...
        title = "{0} Activities Between {1} and {2}".format(
            "Weekly" if weekly else "Daily", stringify_date(start), stringify_date(end)
        )
        if hasattr(self, "weekday_selector"):
