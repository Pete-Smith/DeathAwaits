...
class PieChart(PlotDialogBase):
    ...
    def _plot(self, figure, database, activity, start, end):
        ...
        totals_option = self.totals_box.itemData(self.totals_box.currentIndex())
        if totals_option:
            for i in range(len(labels)):
                if totals_option == 'week':
                    labels[i] += " ({0:.1f} hrs/wk)".format(
                        items_shown[i][1] * 7 * 24
                    )
                    ...
                    labels[i] += " ({0:.1f} hrs)".format(
                        items_shown[i][1] * ((end - start).total_seconds()/60.0/60.0)
                    )
        sizes = [n[1] for n in items_shown]
        if (activity
            and not self.inclusive_other.isChecked()
            and sum(sizes) != 0.0
           ):
            multiplier = abs(1.0 / float(sum(sizes)))
            sizes = [n * multiplier for n in sizes]
        axes.set_title(
            "Time Spent Between {0} and {1}".format(
